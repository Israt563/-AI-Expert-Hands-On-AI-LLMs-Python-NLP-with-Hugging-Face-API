...
# Put your actual Hugging Face API key right here inside the quotes
...
def ask_hf(headline: str):
    ...
    # Retry loop for when the model is cold-loading (503 error)
    for _ in range(3):
        r = requests.post(API_URL, headers=HEADERS, json=payload, timeout=30)
        
        if r.status_code == 503:
            print("⏳ Model is loading on Hugging Face servers, waiting 10 seconds...")
            time.sleep(10)
            continue
    ...
    raise RuntimeError("Model took too long to load. Please try again in a moment.")
...
def show(headline: str, preds: list):
    ...
    print("\n" + "=" * 60)
    print("[INFO] News Topic Classifier")
    print("=" * 60)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            print("Reason:", e)
            print("Tip: Check HF_API_KEY + internet connection.\n")

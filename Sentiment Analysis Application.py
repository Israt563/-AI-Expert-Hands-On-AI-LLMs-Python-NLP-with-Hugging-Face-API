...
# REPLACE THIS WITH YOUR NEW HUGGING FACE API TOKEN
HF_API_KEY = "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"

# Change model to this if required --> "sentence-transformers/all-mpnet-base-v2"
...
clean = lambda t: [
    w
    for w in (re.sub(r"[^a-z0-9']+", "", x.lower()) for x in t.split())
    if w
]
...
def smart_score(base, q1, q2, strong):
  ...
  boost = (
      (0.04 if len(strong) >= 2 else 0)
      + (0.03 if jac >= 0.20 else 0)
      + (0.05 if jac >= 0.35 else 0)
  )
  negA = [
      "not",
      "no",
  ...
  opp_pen = (
      0.12
      if any(
          (has_any(q1, [a]) and has_any(q2, [b]))
          or (has_any(q1, [b]) and has_any(q2, [a]))
          for a, b in oppA
      )
      else 0
  )
...
def label(s):
  return (
      "✅ DUPLICATE"
      if s >= TH
      else ("🤔 CLOSE MATCH" if s >= TH - 0.05 else "❌ DIFFERENT")
  )
...
def show_flow(q1, q2):
  ...
  print(
      "   Strongest:",
      ", ".join(shared) if shared else "No obvious shared/synonym matches",
  )
...
def run(q1, q2, title):
  ...
  base = hf(q1, q2)
  strong = sorted(
      {w for w in clean(q1) if len(w) >= 4}
      & {w for w in clean(q2) if len(w) >= 4}
  )
